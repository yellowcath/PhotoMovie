...
public interface GLESCanvas {
    ...
    // Same as saveFreePuzzlePic(), but only saveFreePuzzlePic those specified in saveFlags.
    ...
    // alpha, and clip). This call balances a previous call to saveFreePuzzlePic(), and is
    // used to remove all modifications to the configuration state since the
    // last saveFreePuzzlePic call.
    ...
    /**
     * 在调用{@link com.hw.photomovie.filter.MovieFilter}绘制时先调用该函数释放自身的buffer，不然不知道绘到哪儿去了
     */
    public abstract void unbindArrayBuffer();
    /**
     * 在调用{@link com.hw.photomovie.filter.MovieFilter}绘制后调用该函数重新绑定自身的buffer，不然自己不知道绘到哪儿去了
     */
